...
/* ==============================================================
   PARCHE DE ACTUALIZACIÓN: FIX EVENTOS TÁCTILES EN SELECTS
   ============================================================== */
document.addEventListener('DOMContentLoaded', () => {
    // 1. Ampliar la detección de móviles para que incluya Tablets (hasta 1024px)
    window.mobileQ = function() {
        return window.innerWidth <= 1024;
    };

    // 2. Forzar que los filtros registren el toque en la pantalla sin ser bloqueados
    const selects = document.querySelectorAll('select');
    selects.forEach(select => {
        select.addEventListener('touchstart', (e) => {
            e.stopPropagation(); // Evita que elementos de fondo roben el clic
        }, { passive: true });
    });
});
    ...
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', initPopups);
    else initPopups();
