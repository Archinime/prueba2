...
/* ----------------------------
   script-index.js (versión corregida)
   ---------------------------- */

/* ----------------------------
    Helpers y render (mantengo tal cual tu lógica)
----------------------------*/
function render(list) {
  const grid = document.getElementById('grid');
  if (!list || list.length === 0) {
...
----------------------------*/
if (typeof animes !== 'undefined') {
  render((typeof shuffleArray === 'function' ? shuffleArray(animes) : animes));
  updateResultsCount(animes.length);
} else {
  console.error("Error: No se encontró la lista 'animes'. Revisa que index-data.js esté bien vinculado."); 
}

document.getElementById('search').addEventListener('input', ev => { debouncedFiltro(); scheduleCheck && scheduleCheck(); });
document.getElementById('search').addEventListener('keydown', (e)=>{ if(e.key === 'Enter'){ e.preventDefault(); filtro(); } });

/* Aseguro que los selects (si existen) disparen filtro al cambiar */
...
/* ----------------------------
   FIX: Mobile select popups (mejor manejo de eventos para evitar scroll accidental)
   Basado en tu módulo original mobileSelectPopups — mantuve la lógica y solo ajusté listeners
----------------------------*/
(function mobileSelectPopups() {
  const mobileQ = () => window.matchMedia('(max-width:780px)').matches;
  const SELECT_IDS = ['genre-select','demographic-select','rating-select'];
  ...
  function openPopupFor(selectEl) {
    ...
    const pad = 8;
    const maxHeight = window.matchMedia('(max-width:420px)').matches ? 220 : 300;
    popup.style.maxHeight = maxHeight + 'px';
    const width = Math.min(rect.width, Math.max(120, winW - 24));
    popup.style.minWidth = Math.max(150, width) + 'px';
    ...
    opts.forEach((opt, idx) => {
      ...
      d.addEventListener('keydown', (ev) => {
        ...
        else if (ev.key === 'ArrowDown') { ev.preventDefault(); const next = d.nextElementSibling; if (next) next.focus(); }
        else if (ev.key === 'ArrowUp') { ev.preventDefault(); const prev = d.previousElementSibling; if (prev) prev.focus(); }
        else if (ev.key === 'Escape') { closePopup(); try { selectEl.focus(); } catch(e){} }
      });
      popup.appendChild(d);
    });
  ...
  function createPopupFor(selectEl) {
    // FIX: prefer pointerdown/touchstart to avoid native focus -> scroll behavior
    const onPointer = function onPointerDown(e) {
      if (!mobileQ()) return;
      // si es pointerdown desde stylus/mouse en algunos browsers, solo abrir en touch-like pointers
    ...
    // Use passive:false for touchstart so preventDefault works
    selectEl.addEventListener('pointerdown', onPointer, { passive: false });
    selectEl.addEventListener('touchstart', onPointer, { passive: false });
    // Fallback: also keep click for devices that don't trigger pointer/touch
    selectEl.addEventListener('click', function onClick(e){
      if (!mobileQ()) return;
      e.preventDefault();
      e.stopPropagation();
  ...
  function init() {
    ...
    } catch(e) { console.warn('mobileSelectPopups init error', e); }
  }

  window.addEventListener('resize', function(){ if (!mobileQ()) closePopup(); }, { passive:true });
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', init);
  else init();
  window._closeMobileSelectPopup = closePopup;
})();
...
/* ----------------------------
   Terminal / No results helpers (mantengo tu lógica)
   ----------------------------*/
function typeWriter(text, element) {
  element.textContent = '';
  let i = 0; clearInterval(element._tw);
  element._tw = setInterval(() => {
    if(i < text.length) { element.textContent += text.charAt(i); i++; } 
    else { clearInterval(element._tw); }
  }, 30);
}
...
function updateTerminalStatus() {
  ...
  if (msgs.length === 0) finalMsg = "> MOSTRANDO_TODO // LINK_STABLE";
  else finalMsg += msgs.join(" | ") + " // BUSCANDO.";
...
function checkEmptyGrid() {
  ...
  let visibleCount = 0;
  cards.forEach(c => {
    if (c.style.display !== 'none' && !c.classList.contains('hidden')) visibleCount++;
  });
  if (visibleCount === 0) {
    noResultsBox.style.display = 'flex';
    gridEl.style.display = 'none';
    typeWriter("> ADVERTENCIA // 0 COINCIDENCIAS_EN_LA_MATRIZ", terminalOut);
  } else {
...
function scheduleCheck() {
  ...
  setTimeout(checkEmptyGrid, 100);
}

document.getElementById('search').addEventListener('input', scheduleCheck);
document.getElementById('genre-select').addEventListener('change', scheduleCheck);
document.getElementById('demographic-select').addEventListener('change', scheduleCheck);
document.getElementById('rating-select').addEventListener('change', scheduleCheck);
