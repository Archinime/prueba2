...
// FIX: Aquí está la corrección principal para que el botón no quede activo visual o lógicamente de forma equivocada.
async function enviarComentarioTexto() {
    ...
    // Al limpiar la caja, evaluamos de inmediato para forzar su estado CSS.
    ...
    try {
        ...
        // Vuelve a calcular estado dependiendo si todo fue limpio (éxito) o hay texto (error)
        validarBotonPrincipal(textoInput); 
    }
}
...
// FIX: Igualmente arreglamos la lógica de botón desactivado en respuestas
window.enviarRespuestaDinamica = async function() {
    ...
    try {
        ...
        cancelarRespuesta(true); // <--- Esto ya destruye la caja, así que no hay que resetear botones
        showToastComent('✅ Respuesta enviada');
        
    } catch (error) {
        alert('Error: ' + error.message);
        // Si hay error, hay que volver a habilitar la caja
