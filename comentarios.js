...
object-fit: contain; }
        ...
        .comentario-sticker-preview {
            margin: 12px 0;
padding: 8px; background: rgba(0, 0, 0, 0.4); border-radius: 16px; border: 1px solid rgba(0, 243, 255, 0.3); display: flex; align-items: center;
gap: 12px; max-width: 100%; overflow: hidden;
        }
        .preview-sticker-wrapper {
            position: relative;
display: inline-block; max-width: 80px; max-height: 80px; background: rgba(0,0,0,0.6); border-radius: 12px; overflow: hidden; border: 1px solid var(--cm-neon-primary);
}
        .preview-sticker-wrapper img,
        .preview-sticker-wrapper video {
            width: auto;
height: auto; max-width: 80px; max-height: 80px; object-fit: contain; display: block; margin: 0 auto;
}
        .remove-sticker-btn {
            position: absolute;
top: -8px; right: -8px; background: #ff0055; border: none; color: white; border-radius: 50%; width: 24px; height: 24px; font-size: 12px; cursor: pointer;
display: flex; align-items: center; justify-content: center; box-shadow: 0 0 8px rgba(255,0,85,0.6); transition: transform 0.2s;
}
...
if (typeof procesarReaccionesHTML === 'function') reaccionesBar = procesarReaccionesHTML(c.id, c.reactions);
    ...
    const encodedText = encodeURIComponent(c.texto || '');
    const botonResponder = currentUser ?
`<button class="btn-responder-ghost" onclick="prepararRespuesta('${c.id}', '${escapeHtmlComent(userName)}', '${c.userId}', '${encodedText}'); closeAllCommentMenus();">Responder</button>` : '';

    return `
        <div class="comentario-item ${isReply ? 'is-reply' : ''} ${newFxClass}" id="comment-${c.id}" 
            ondblclick="prepararRespuesta('${c.id}', '${escapeHtmlComent(userName)}', '${c.userId}', '${encodedText}'); closeAllCommentMenus();"
...
window.prepararRespuesta = function(commentId, userName, userId, encodedTextoOriginal) {
    const currentUser = getCurrentUser();
...
restaurarPanelesGlobales(); 
    ...
    try {
        ...
        texto: textoFinal,
            ...
            replyToText: replyContext.texto || null,
