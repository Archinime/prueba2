...
// CON INICIALIZACIÓN ROBUSTA, SANITIZACIÓN Y ESTADO CENTRAL
// ============================================
...
function initComentariosSystem(db, auth) {
    ...
    const interval = setInterval(() => {
        if (tryStartListener() || retry > 30) clearInterval(interval);
        retry++;
    }, 1000);
}
...
function injectCommentsCSS() {
    ...
    style.innerHTML = `...`; // (El CSS completo está en tu archivo original, lo mantienes igual)
...
function setupComentariosRealtimeListener() {
    ...
    if (!window.comentariosAnimeId || window.comentariosSeason === undefined || window.comentariosEpisode === undefined) return;
...
// ========== GENERAR HTML DE COMENTARIO (CON SANITIZACIÓN) ==========
...
function updateComentariosUI() {
    ...
    if (!currentUser) {
        ...
        if (avatar) avatar.src = currentUser.photoURL || 'invitado.avif';
        const nameSpan = document.getElementById('comentarioUserName');
        if (nameSpan) nameSpan.innerText = currentUser.displayName || currentUser.email.split('@')[0];
    }
}
