...
async function subirAGithHub() {
    ...
    try {
        ...
        // ============================================
        // LÓGICA DE DETECCIÓN DE DATOS PARA NOTIFICACIÓN
        // ============================================
        let lastSeasonCover = nuevoAnime.portada;
        let lastBlockName = "Novedad";
        let lastEpTitle = "Nuevo Contenido";
        let detectedLangs = []; // Array para guardar idiomas (lat, sub)
        ...
        if (nuevoAnime.temporadas && nuevoAnime.temporadas.length > 0) {
            ...
            if (lastSeason.eps && lastSeason.eps.length > 0) {
                ...
                // AQUÍ DETECTAMOS SI EL ÚLTIMO CAP ES LAT, SUB O AMBOS
                if (lastEp.link && lastEp.link.trim() !== "") detectedLangs.push('lat');
                if (lastEp.link2 && lastEp.link2.trim() !== "") detectedLangs.push('sub');
            }
        }
        // ============================================
        ...
        await updateGithubFile(token, OWNER, REPO, 'index-data.js', (content) => {
            ...
            const newIndexEntry = {
                ...
                langs: detectedLangs, // ¡GUARDAMOS EL IDIOMA AQUÍ!
